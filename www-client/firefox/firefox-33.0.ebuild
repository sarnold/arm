...
src_prepare() {
	...
	# add upstream patches for arm:
	# https://bugzilla.mozilla.org/show_bug.cgi?id=956606
	#epatch "${FILESDIR}"/${PN}-29.0.1-armhf.patch
...
src_compile() {
	use arm && cp "${FILESDIR}"/autoconf.mk.gentoo \
		"${S}"/obj-armv7a-hardfloat-linux-gnueabi/config/autoconf.mk
	sed -i -e "/#define CROSS_COMPILE 1/d" \
		-e "s|GL_PROVIDER_GLX|GL_PROVIDER_EGL|"
		${S}"/obj-armv7a-hardfloat-linux-gnueabi/mozilla-config.h
