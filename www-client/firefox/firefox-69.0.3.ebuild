...
src_prepare() {
	...
	use arm && eapply "${FILESDIR}/${PN}"-68.0.2-arm-fp-wasm-fixes.patch \
		"${FILESDIR}/${PN}"-68.0.2-use-neon-flags-for-libaom.patch
...
src_configure() {
	...
	filter-flags -flto*
	use arm && filter-flags -mfpu* -mfloat-abi*
	...
	if use neon ; then
		...
		if ! tc-is-clang ; then
			...
			mozconfig_annotate '' --with-thumb=no
			mozconfig_annotate '' --with-thumb-interwork=no
		fi
	else
		# fall back to generic armv7-a vfp
		mozconfig_annotate '' --with-fpu=vfpv3-d16
	fi
	...
	if use arm; then
		...
		mozconfig_annotate '' --target="${rust_target}"
		mozconfig_annotate '' --host="${rust_target}"
	else
