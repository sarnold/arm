...
LICENSE="BSD"
SLOT="0/3"
IUSE="cpu_flags_x86_avx cpu_flags_x86_avx2 doc cpu_flags_x86_mmx neon postproc cpu_flags_x86_sse cpu_flags_x86_sse2 cpu_flags_x86_sse3 cpu_flags_x86_ssse3 cpu_flags_x86_sse4_1 static-libs svc test +threads"
...
multilib_src_configure() {
	...
	local myconf
	if [[ ${ARCH} == arm* ]] ; then
		cputune=$(get-flag mtune)
		fputune=$(get-flag mfpu)
		[ -n ${cputune} ] && myconf+=" --cpu=${cputune}"
		if use neon ; then
			myconf+=" --target=armv7-linux-gcc --enable-thumb"
		else
			myconf+=" --target=armv6-linux-gcc"
		fi
	fi
