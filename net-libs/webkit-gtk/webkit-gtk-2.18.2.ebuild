...
IUSE="aqua coverage doc +egl +geolocation gles2 glx gnome-keyring +gstreamer +introspection +jit libnotify neon nsplugin +opengl spell wayland +webgl X"
...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-2.8.5-fix-ia64-build.patch \
		"${FILESDIR}"/${PN}-arm-asm.patch \
		"${FILESDIR}"/${PN}-bug153373.patch
#		"${FILESDIR}"/${PN}-relax-ruby-version-and-fix-offsets.patch
...
src_configure() {
	...
	if use opengl ; then
		local mycmakeargs+=( -WTF_USE_OPENGL=1 )
		if use glx ; then
	...
	# testing...
#	mycmakeargs+=( -DDISABLE_STRICT_BUILD=ON )
